...
type UserFormProps = {
  setUpdateUserModalOpen: Dispatch<SetStateAction<boolean>>;
  loading: boolean;
  userDetail: Partial<IUser>;
  handleUpdateUser: (id: number, user: Partial<IUser>) => void;
...
export default function UserUpdateForm({
  ...
  const onSubmit = (data: Partial<IUser>) => {
    handleUpdateUser(userDetail.id as number, data);
  };

  const initialValues: FormValues<any> = {
    id: userDetail.id,
  ...
  return (
    <div className="min-h-[300px] mx-auto mt-8 flex flex-col">
      {!loading && (
        <span
          onClick={() => handleDeleteUser(userDetail.id as number)}
          className="cursor-pointer material-icons rounded-full absolute top-5 right-0 text-red-500 text-3xl"
        >
          delete_outlined
        </span>
      )}
